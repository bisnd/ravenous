...
async function Yelp (searchLocation, searchTerm, sortingOption) {
    ...
    try{
        const response = await fetch(`https://cors-anywhere.herokuapp.com/https://api.yelp.com/v3/businesses/search?location=${searchLocation}&term=${searchTerm}&sort_by=${sortingOption}&limit=20`, options);
        if (response.ok) {
